...
class ActivityAE
{
  function AddActivity(){
    ...
    $checker = true;
    while ($checker==true) {
      if(!$activityAE->ValidateInput($AcDescription)){$checker = false; $err='Invalid description'; break;}
      if(!$activityAE->ValidateInputDate($AcDate)){$checker = false; $err='Invalid date, write as: dd-mm-yyyy '; break;}
      if(!$activityAE->ValidateInputTime($AcStartTime)){$checker = false; $err='Invalid start time, write as hh:mm'; break;}
      if(!is_numeric($AcVolReq)){$checker = false; $err='Invalid volunteer number, make sure only numbers are used'; break;}
      if(!$activityAE->ValidateInput($AcLocation)){$checker = false; $err='Invalid Location, use only text, numbers, hyphons or colons'; break;}
      break;
    }
  ...
  function ChangeActivity(){
    ...
    $checker = true;
    while ($checker==true) {
      if(!$activityAE->ValidateInput($CADescription)){$checker = false; $err='Invalid description'; break;}
      if(!$activityAE->ValidateInputDate($CADate)){$checker = false; $err='Invalid date, use hyphons'; break;}
      if(!$activityAE->ValidateInputTime($CAStartTime)){$checker = false; $err='Invalid start time, use colons'; break;}
      if(!is_numeric($CAVolReq)){$checker = false; $err='Invalid volunteer number, make sure only numbers are used'; break;}
      if(!is_numeric($CAID)){$checker = false; break; header("location: ActivityAEView.php");}
      if(!$activityAE->ValidateInput($CALocation)){$checker = false; $err='Invalid Location, use only text, numbers, hyphons or colons'; break;}
      break;
    }
    ...
    if($checker == false){
      ...
      $EmptyEditActivity = new stdClass();
      $EmptyEditActivity->Description = ' ';
      $EmptyEditActivity->Time = ' ';
      $EmptyEditActivity->AmountOfVolunteers = ' ';
      $EmptyEditActivity->Location = ' ';

      CMS_EditActivity($CADescription, $CADate, $CAStartTime, $CALocation, $CAVolReq, $_GET['ActivityId']);
      $_SESSION['ActivityAEEditActivity'] = $EmptyEditActivity;
