...
function GetActivities($filter = NULL, $change = false)
{
    ...
    if ($filter != NULL && $change == false) {
        ...
        $stmt->prepare('SELECT A.ActivityId, A.Description, Time, A.Location, AmountOfVolunteers, AmountOfVolunteers - COUNT(AV.ActivityID) AS VolunteersRequired FROM Activity AS A LEFT JOIN ActivityVolunteer AS AV ON AV.ActivityID = A.ActivityID WHERE A.Description LIKE ? OR A.Location LIKE ? OR A.AmountOfVolunteers LIKE ? OR A.ActivityId LIKE ? ORDER BY A.AmountOfVolunteers DESC');
    ...
    if ($filter != NULL && $change == true) {
        ...
        global $mysqli;
        $stmt = $mysqli->stmt_init();
        $stmt->prepare('SELECT A.ActivityId, A.Description, Time, A.Location, AmountOfVolunteers, AmountOfVolunteers - COUNT(AV.ActivityID) AS VolunteersRequired FROM Activity AS A LEFT JOIN ActivityVolunteer AS AV ON AV.ActivityID = A.ActivityID WHERE A.ActivityId LIKE ?');
        $stmt->bind_param("s", $filter);
        $stmt->execute();
        $stmt = $stmt->get_result();
        foreach ($stmt as $item) {
            $activity = new ActivityModel($item["ActivityId"], $item["Description"], $item["Time"], $item["Location"], $item["AmountOfVolunteers"], $item["VolunteersRequired"]);
            return $activity;
            break;
        }
    }
    ...
    if ($stmt != null) {
        foreach ($stmt as $item) {
            $tempItem = new ActivityModel($item["ActivityId"], $item["Description"], $item["Time"], $item["Location"], $item["AmountOfVolunteers"], $item["VolunteersRequired"]);
            array_push($activityList, $tempItem);
        }
    }
    return $activityList;
}

function CMS_AddActivity($AcDescription, $AcDate, $AcStartTime, $AcVolReq, $AcLocation)
{
...
function CMS_GetVolunteers()
{
    ...
    if ($stmt != '') {
        foreach ($stmt as $tempVolunteer){
            $volunteer = new VolunteerModel($tempVolunteer['VolunteerID'], $tempVolunteer['Activities'], $tempVolunteer['Function'],$tempVolunteer['FirstName'], $tempVolunteer['LastName'], $tempVolunteer['Email'], $tempVolunteer['RegisterDate']);
            array_push($volunteerList, $volunteer);
        }
    }
